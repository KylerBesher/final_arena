...
export function processSectionStyles(style) {
    ...
    if(style.colors) {
        ...
        if(lightMode) {
            ...
            if(lightMode.background) {
                ...
                debug.log({
                    originalColor: lightMode.background,
                    processedColor: bgColor,
                    finalClass: bgClass,
                    charArray: [...bgColor],
                    asciiCodes: [...bgColor].map((c) => c.charCodeAt(0)),
                });

                classes.push(bgClass);
                classes.push(`dark:bg-[${darkMode?.background.toUpperCase() || lightMode.background.toUpperCase()}]`);
            }
            ...
            if(darkMode) {
                ...
                if(darkMode.text) {
                    ...
                    classes.push(`dark:text-[#${darkTextColor.replace('#', '')}]`);
                }
            }
        }
    }
    ...
    debug.log('Final classes:', classes);
